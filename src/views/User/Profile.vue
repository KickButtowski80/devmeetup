...
export default {
  data() {
    return {
      imgUrl: this.$store.state.user ? this.$store.state.user.photoURL : ""
    };
  },
  created() {
  ...
  computed: {
    ...
    registeredMeetups() {
      ...
      for (i = 0; i < rm.length; i++) {
        ...
        for (j = 0; j < allm.length; j++) {
          ...
          if (allm[j].id == rm[i].toString()) meetupsInfo.push(allm[j]);
        }
      }
    ...
    meetups() {
      return this.$store.getters.loadedMeetups.filter(
        meetup => meetup.creatorId === this.$store.getters.user.uid
      );
    },
    profilesInfo() {
